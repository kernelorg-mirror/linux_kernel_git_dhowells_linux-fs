			      ...
			      uoff_t fpos1, uoff_t fpos2)
{
	...
	netfs_rmw_read_one(rreq, bq, fpos1);
	if (bq->next)
				    ...
				    uoff_t to, uoff_t end)
{
	...
	if (subreq->start < wreq->start) {
		before = bvecq_alloc_one(2, wreq->gfp, false);
		if (!before)
			return -ENOMEM;
		fpos1 = subreq->start;
	...
	if (to == end && subreq->start + subreq->len < to) {
		...
		after = bvecq_alloc_one(1, wreq->gfp, false);
		if (!after) {
			ret = -ENOMEM;
			goto out;
		}
...
do_it:
	ret = netfs_rmw_read(wreq, before ?: after, fpos1, fpos2);
					   ...
					   struct netfs_group *netfs_group)
{
	 ...
	 * shorten the request.
	 */
	n = netfs_extract_iter(iter, len, INT_MAX,
	...
	if (test_bit(NETFS_RREQ_USE_BOUNCE_BUFFER, &wreq->flags)) {
		...
		ret = bvecq_buffer_init(&wreq->bounce_alloc, wreq->gfp, false);
		if (ret < 0)
			goto error_put;
